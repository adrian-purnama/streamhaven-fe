...
export default function FeedbackForm({ onSubmit, onCancel, feedbackType = 'feedback' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      const payload = { feedback: text, feedbackType }
      if (needsRecaptcha) payload.recaptchaToken = recaptchaRef.current.getValue()
      await apiHelper.post('/api/feedback', payload)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {needsRecaptcha && (
        <div className="flex justify-center [&_.grecaptcha-badge]:bottom-14!">
          <ReCAPTCHA ref={recaptchaRef} sitekey={recaptchaSiteKey} theme="dark" size="normal" />
        </div>
      )}
      {userId && (
