...
import TopPickForm from '../../../components/forms/TopPickForm'
...
const MovieSyncPage = () => {
  ...
  const fetchMovies = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || err.message || 'Failed to load movies')
  ...
  const fetchLastSync = useCallback(async () => {
    try {
      const res = await apiHelper.get('/api/movies/sync')
  ...
  useEffect(() => {
    fetchMovies()
    fetchLastSync()
  }, [fetchMovies, fetchLastSync])

  const handleSync = async () => {
    setSyncing(true)
    try {
      const res = await apiHelper.post('/api/movies/sync', { override })
    ...
    <div className="min-h-screen bg-gray-900 px-4 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <h1 className="text-2xl font-semibold text-gray-100">Movie sync</h1>
        ...
        <div className="rounded-xl border border-gray-700 bg-gray-800 overflow-hidden">
          {activeTab === 'top_pick' ? (
            ...
            <div className="px-4 py-12 text-center text-gray-400">
              No movies in this category. Click &quot;Sync movies&quot; to load from TMDB.
            </div>
          ) : (
