...
const emptyData = { now_playing: [], popular: [], top_rated: [], top_pick: [] }
...
export default function MoviePage() {
  ...
  const fetchCategoryPage = useCallback(
    async (cat, page) => {
      const res = await apiHelper.get(`/api/movies/${cat}`, { params: { page } })
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white px-4 py-8 max-w-6xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <Link to="/movie" className="text-gray-400 hover:text-white text-sm">
            ← Movies
          </Link>
          ...
          <>
            ...
            <PosterGrid>
              {categoryResults.map((item) => (
                <Poster
                  ...
                  movie={{ ...item, externalId: item.id ?? item.externalId, title: item.title }}
                  size="md"
                  mediaType="movie"
                  className="shrink-0"
                />
            ...
            {categoryTotalPages > 1 && (
              <div className="flex justify-center gap-2 mt-8">
                <Link
                  to={`/movie/${category}?page=${Math.max(1, pageFromUrl - 1)}`}
                ...
                <Link
                  to={`/movie/${category}?page=${Math.min(categoryTotalPages, pageFromUrl + 1)}`}
                  className={`px-4 py-2 rounded-lg bg-gray-700 text-white hover:bg-gray-600 transition-colors ${
